...
void CEDCPartialMerkleTree::TraverseAndBuild(int height, unsigned int pos, const std::vector<uint256> &vTxid, const std::vector<bool> &vMatch) 
{
...
uint256 CEDCPartialMerkleTree::TraverseAndExtract(int height, unsigned int pos, unsigned int &nBitsUsed, unsigned int &nHashUsed, std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) 
{
...
uint256 CEDCPartialMerkleTree::ExtractMatches(std::vector<uint256> &vMatch, std::vector<unsigned int> &vnIndex) 
{
    ...
    if (nTransactions > EDC_MAX_BLOCK_SIZE / 60) // 60 is the lower bound for the size of a serialized CTransaction
        return uint256();
