...
UniValue edcgetmininginfo(const UniValue& params, bool fHelp)
{
	...
	obj.push_back(Pair("currentblockcost", (uint64_t)theApp.lastBlockCost() ));
...
UniValue edcgetblocktemplate(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (CEDCTransaction& tx, pblock->vtx) 
	{
        ...
        entry.push_back(Pair("cost", edcGetTransactionCost(tx)));
    ...
    result.push_back(Pair("sigoplimit", (int64_t)EDC_MAX_BLOCK_SIGOPS_COST));
    result.push_back(Pair("sizelimit", (int64_t)EDC_MAX_BLOCK_SERIALIZED_SIZE));
    result.push_back(Pair("costlimit", (int64_t)EDC_MAX_BLOCK_COST));
